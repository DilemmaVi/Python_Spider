...
def url_get(spider_name,thread_lock,court_list):

	time.sleep(spider_name/2)
	# 目标网站地址
			...
			while court_index<101:
				...
				try:
					...
					if len(CountResult)>0:
						...
						#若已经获取到足够的数据就退出查询此法院
						if int(CountResult[0])-int(cc_dic[x])<=CourtInput_count or court_index==100:
							cc_dic[x]=CountResult[0]
							thread_lock.acquire()
							SqlHelper.UdateTotalCount(x, CountResult[0])
							thread_lock.release()
							print(LocalTime()+'yf爬虫'+str(spider_name)+'已获取到足够数据，跳入下一个法院')
							SpiderLog.writeIndfoLog(LocalTime()+'yf爬虫'+str(spider_name)+'已获取到足够数据，跳入下一个法院')
							break
						else:
							court_index+=1
					else:
