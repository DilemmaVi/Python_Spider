...
while True:
	try:
		...
		print(LocalTime()+':本次总共需要爬取'+str(len(DataCount))+'个法院的案件链接')
		SpiderLog.writeIndfoLog('本次总共需要爬取'+str(len(DataCount))+'个法院的案件链接')
		...
		for x in DataCount: 
			...
			while Index <=count:
				...
				try:
					...
					SpiderLog.writeErrorLog(data+traceback.format_exc())
				...
				time.sleep(SleepNum)
				if CountData:
					print(LocalTime()+':该法院未查到增量数据，跳入爬取下一个法院')
					# NOdata.append(x)
					break
				else:
					Index+=1
